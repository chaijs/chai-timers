...
module.exports = function (chai, _) {
  ...
  _.overwriteMethod(chai.Assertion, 'before', function (_super) {
    return function assertBefore (timer2, when2) {
      ...
      this.assert(
          time1 < time2
        , 'expected timer {' + timer1.name + '} to have ' + when1 + ' before timer {' + timer2.name + '} ' + when2
        , 'expected timer {' + timer1.name + '} to not have ' + when1 + ' before timer {' + timer2.name + '} ' + when2
      );
    };
  });
  ...
  _.overwriteMethod(chai.Assertion, 'after', function (_super) {
    return function assertBefore (timer2, when2) {
      ...
      this.assert(
          time1 < time2
        , 'expected timer {' + timer1.name + '} to have ' + when1 + ' after timr {' + timer2.name + '} ' + when2
        , 'expected timer {' + timer1.name + '} to not have ' + when1 + ' after timer {' + timer2.name + '} ' + when2
      );
    };
  });
